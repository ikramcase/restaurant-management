...
func GetOrderItems() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occue while listening ordered items"})
			return
		}
...
func GetOrderItemsByOrder() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occue while listening order items by id"})
			return
		}
		c.JSON(http.StatusOK, allOrderItems)
	}
...
func ItemsByOrder(id string) (OrderItem []primitive.M, err error) {
	...
	groupStage := bson.D{{"$group", bson.D{{"$_id", bson.D{{"order_id", "$order_id"}, {"table_id", "$table_id"}, {"table_number", "$table_number"}}}, {"payment_due", bson.D{{"$sum", "$amount"}}}, {"total_count", bson.D{{"$sum", 1}}}, {"order_item", bson.D{{"$push", "$$ROOT"}}}}}}
...
func GetOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occur while listing order item"})
			return
		}
